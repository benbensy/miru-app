...
class ExtensionBrowsePage extends GetCommonWidget<ExtensionBrowseController> {
  ...
  @override
  void initState(GetBuilderState<ExtensionBrowseController> state) {
  ...
  @override
  void dispose(GetBuilderState<ExtensionBrowseController> state) {
    controller.isPageOpen = false;
  }

  // 导入扩展对话框
  _importDialog(BuildContext context) {
    String url = '';
    showPlatformDialog(
      context: context,
  ...
  // 加载错误对话框
  _loadErrorDialog(BuildContext context) {
    showPlatformDialog(
      context: context,
      ...
      content: SingleChildScrollView(
        child: Column(
          ...
          children: [
            // 输出key 和 value
            for (final e in controller.errors.entries)
              PlatformWidget(
                mobileWidget: Card(
          ...
          actions: [
            if (controller.errors.isNotEmpty)
              IconButton(
                ...
                onPressed: () => _loadErrorDialog(context),
              ),
            IconButton(
